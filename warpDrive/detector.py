...
class detector:
    ...
    def __init__(self, dshape, ditemsize, dfilterBig, dfilterSmall):
        ...
        self.thresh = np.float32(20)
        # FIXME: decide reasonable size for maximum filter
        self.halfMaxFilt = np.int32(5)  # maxFilter size will be 2*halfMaxFilt + 1
    ...
    def allocateMem(self):
        ...
        self.FOVsize = int(self.dshape[0])
        self.calcCRLB = np.int32(0)

    #@classmethod
    def prepvar(self, varmap):
        self.varmap = varmap
    ...
    def getCand(self):
        ...
        self.findpeaks(self.unif1_gpu, self.maxfData_gpu, self.thresh, self.colsize, self.candCount_gpu,
                       self.candPos_gpu, block=(self.rsize, 1, 1), grid=(self.csize, 1), stream=self.dstreamer1)
    ...
    def fitItSlow(self):
        ...
        self.CRLBs = np.zeros(6*self.candCount, dtype=np.float32)
        self.CRLBs_gpu = cuda.mem_alloc(self.CRLBs.size*self.CRLBs.dtype.itemsize)
        cuda.memcpy_htod(self.CRLBs_gpu, self.CRLBs)
        ...
        self.gaussAstig(self.data_gpu, np.float32(1.4), np.int32(self.ROIsize), np.int32(200),# FIXME: note, second ROIsize would normally be FOV size
                        self.dpars_gpu, self.CRLBs_gpu, self.LLH_gpu, self.candCount_gpu, self.invvar_gpu, self.gain_gpu,
        ...
        cuda.memcpy_dtoh(self.dpars, self.dpars_gpu)
        self.dpars = np.reshape(self.dpars, (6, self.candCount))
        #self.fitpars = np.reshape(self.dpars, (self.candCount, 6))

        cuda.memcpy_dtoh(self.CRLBs, self.CRLBs_gpu)
        cuda.memcpy_dtoh(self.LLH, self.LLH_gpu)
