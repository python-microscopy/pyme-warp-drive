...
__device__ void kernel_sum_reduce(const int BlockSize, const int pixelIndex, volatile float *A, volatile float *Aout) {
    ...
    */
	...
	// Mod AESB 04222016 to work as first iterations for block size <128 
    ...
    //if ((pixelIndex < 32)){
	if ((BlockSize >= 32) && (pixelIndex < min(32, BlockSize - 32))){
...
__global__ void kernel_MLEFit_pix_threads(float *d_data, float PSFSigma, int sz, int iterations, 
    ...
    */
    ...
    //Allocate arrays for the gradients etc ... Change to global variables YL20160408
    float dudt[NUM_VARS];
    ...
    if (pixelIndex < NUM_VARS){
		...
		d_Parameters[Nfits*pixelIndex + blockIdx.x] = theta[pixelIndex];
		d_CRLBs[Nfits*pixelIndex + blockIdx.x] = Diag[pixelIndex];
    }
    
    return;
}

__global__ void kernel_MLEFit_pix_threads_sigma(float *d_data, float PSFSigma, int sz, int iterations, 
        float *d_Parameters, float *d_CRLBs, float *d_LogLikelihood,int Nfits, float *d_varim, float *d_gainim, int calcCRB){
    /* A version of MLEFit that uses per-pixel, rather than per fit threads 
    ...
    */
    ...
    //allocate arrays for matrices
    float M[NV_PS*NV_PS];
    __shared__ float Diag[NV_PS];
    float Minv[NV_PS*NV_PS];
	...
	// it is wrong if shared memory was used
    //__shared__ float dudt[NV_PS];
    //__shared__ float d2udt2[NV_PS];
    ...
    //The numerator and denominator for the update step. These are used from multiple threads, and need
    //to be shared
    volatile __shared__ float NR_Numerator[NV_PS];
    volatile __shared__ float NR_Denominator[NV_PS];

    //the parameter vector
    volatile __shared__ float theta[NV_PS];

    //can these be const??? I don't see maxjump being revised anywhere
    const float maxjump[NV_PS]={1e0, 1e0, 1e2, 2e0, 5e-1}; // maxjump is revised such that the intensity can converge to the correct value
    const float gamma[NV_PS]={1.0, 1.0, 0.5, 1.0, 1.0};
    ...
    for (kk=0;kk<iterations;kk++) {//main iterative loop
        ...
        PSFy=kernel_IntGauss1D(threadIdx.y, theta[1], theta[4]);
        ...
        kernel_DerivativeIntGauss1D(threadIdx.y, theta[1], theta[4], theta[2], PSFx, &dudt[1], &d2udt2[1]);
        //kernel_DerivativeIntGauss2DSigma(threadIdx.x, threadIdx.y, theta[0], theta[1], theta[4], theta[2], PSFx, PSFy, &dudt[4], NULL);
		kernel_DerivativeIntGauss2DSigma(threadIdx.x, threadIdx.y, theta[0], theta[1], theta[4], theta[2], PSFx, PSFy, &dudt[4], &d2udt2[4]);
		dudt[2] = PSFx*PSFy;
        d2udt2[2] = 0.0;
        dudt[3] = 1.0;
        d2udt2[3] = 0.0;
    ...
    if (calcCRB){
        ...
        //Note that if fit is converged, we could probably skip this step
        //as model, dudt etc ... should still have the correct values from the last fit iteration
		//need to check wether use PSFSigma or theth[4]
        PSFx=kernel_IntGauss1D(threadIdx.x, theta[0], PSFSigma);
        PSFy=kernel_IntGauss1D(threadIdx.y, theta[1], PSFSigma);
        ...
        //calculating derivatives
		//change PSFSigma to theta[4] YL 20160408
        //kernel_DerivativeIntGauss1D(threadIdx.x, theta[0], PSFSigma, theta[2], PSFy, &dudt[0], NULL);
        //kernel_DerivativeIntGauss1D(threadIdx.y, theta[1], PSFSigma, theta[2], PSFx, &dudt[1], NULL);
        kernel_DerivativeIntGauss1D(threadIdx.x, theta[0], theta[4], theta[2], PSFy, &dudt[0], NULL);
        kernel_DerivativeIntGauss1D(threadIdx.y, theta[1], theta[4], theta[2], PSFx, &dudt[1], NULL);
		// why it is Null here???
        kernel_DerivativeIntGauss2DSigma(threadIdx.x, threadIdx.y, theta[0], theta[1], theta[4], theta[2], PSFx, PSFy, &dudt[4], NULL);
        dudt[2] = PSFx*PSFy;
        dudt[3] = 1.0;
        ...
        //Building the Fisher Information Matrix
        for (kk=0;kk<NV_PS;kk++) for (ll=kk;ll<NV_PS;ll++){

            //reuse s_temp0 here to save memory
            s_temp0[pixelIndex] = dudt[ll]*dudt[kk]/model;

            //sum over all pixels
            kernel_sum_reduce(BlockSize, pixelIndex, s_temp0, &M[kk*NV_PS+ll]);

            if (pixelIndex == 0){
                //execute in first thread of block only

                //exploit known matrix symetry and copy value to the mirrored entry
                M[ll*NV_PS+kk] = M[kk*NV_PS+ll];
            }
    ...
    */
    ...
    for (kk=0;kk<iterations;kk++) {//main iterative loop
        ...
        if (ll < NUM_VARS_ASTIG){
			// not shown in Fang's code??? YL 20160408
            //if (kk<2)
            //    theta[ll]-=gamma[ll]*min(max(NR_Numerator[ll]/NR_Denominator[ll], -maxjump[ll]), maxjump[ll]);
            //else
                theta[ll]-=min(max(NR_Numerator[ll]/NR_Denominator[ll], -maxjump[ll]), maxjump[ll]);
    ...
    if (calcCRB){
        ...
        //calculating derivatives
		//use theta[4] and thetha[5] instead of PSFSigma in Fang's code
        //kernel_DerivativeIntGauss1D(threadIdx.x, theta[0], PSFSigma, theta[2], PSFy, &dudt[0], NULL);
        //kernel_DerivativeIntGauss1D(threadIdx.y, theta[1], PSFSigma, theta[2], PSFx, &dudt[1], NULL);
		kernel_DerivativeIntGauss1D(threadIdx.x, theta[0], theta[4], theta[2], PSFy, &dudt[0], NULL);
