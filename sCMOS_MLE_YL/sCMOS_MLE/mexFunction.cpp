...
extern "C" void kernel_MLEFit_pix_threads_sigma_wrapper(dim3 dimGrid, dim3 dimBlock, float *d_data, float PSFSigma, int sz, int iterations, 
        float *d_Parameters, float *d_CRLBs, float *d_LogLikelihood,int Nfits, float *d_varim, float *d_gainim, int calcCRB);
...
void mexFunction(int nlhs, mxArray *plhs[],	int	nrhs, const	mxArray	*prhs[]) {
   ...
   // int BlockSize = (int) floor((float)16000/4/sz/sz/4);
	    int BlockSize = (int) floor((float)15000/4/sz/sz/3);
    BlockSize = max(2, BlockSize);
    ...
    switch(fittype){
		...
		case 5: // thread parallel version of 1
            ...
            plhs[1] = mxCreateNumericMatrix(Nfitraw, NV_P, mxSINGLE_CLASS, mxREAL);
            break;
		case 6: // thread parallel version of 1
			cudaMalloc((void**)&d_Parameters,   NV_PS*Nfits*sizeof(float));
            cudaMalloc((void**)&d_CRLBs,        NV_PS*Nfits*sizeof(float));
            ...
            plhs[1] = mxCreateNumericMatrix(Nfitraw, NV_PS, mxSINGLE_CLASS, mxREAL);
            break;
		case 7://thread parallel version of 4 (astigmatic)
			cudaMalloc((void**)&d_Parameters,   NV_PS2*Nfits*sizeof(float));
            cudaMalloc((void**)&d_CRLBs,        NV_PS2*Nfits*sizeof(float));       
            plhs[0]=mxCreateNumericMatrix(Nfitraw, NV_PS2, mxSINGLE_CLASS, mxREAL);
            plhs[1]=mxCreateNumericMatrix(Nfitraw, NV_PS2, mxSINGLE_CLASS, mxREAL);
    ...
    switch(fittype) {
        ...
        case 3: //fit x,y,bg,I,z  
            kernel_MLEFit_z_wrapper(dimGrid, dimBlock, d_data, PSFSigma, Ax, Ay, Bx, By, gamma, d, PSFSigma_y, (int) sz, iterations, d_Parameters, d_CRLBs, d_LogLikelihood, Nfits, d_varim,d_gainim);
            //mexPrintf("A:%f B:%f gamma:%f d:%f \n",Ax,Bx,gamma,d);
            break;

       //******************* Sigma fit does not work for this version of sCMOS software *****************************************//     
        case 4: //fit x,y,bg,I,sigmax,sigmay
            kernel_MLEFit_sigmaxy_wrapper(dimGrid, dimBlock, d_data, PSFSigma, (int) sz, iterations, d_Parameters, d_CRLBs, d_LogLikelihood, Nfits, d_varim, d_gainim);
			break;
		...
		case 6: // do thread parallel version
			 {
                dim3 dimBlock(size_x, size_y);
                dim3 dimGrid(Nfitraw);
                kernel_MLEFit_pix_threads_sigma_wrapper(dimGrid, dimBlock, d_data, PSFSigma, (int) sz, iterations, 
                    d_Parameters, d_CRLBs, d_LogLikelihood,Nfits, d_varim, d_gainim, calcCRB);
            }
            break;

        case 7: // do thread parallel version
            {
                dim3 dimBlock(size_x, size_y);
                dim3 dimGrid(Nfitraw);
